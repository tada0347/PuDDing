...
def sleb(
        ...
        nsamples: int = 128,
        removal_list: list = [],
        folder: str = 'result/5_from10',
        eval_ppl: bool = False,
        eval_zeroshot: bool = True,
        idx: int=0,
        data='winogrande',
        method='likelihood_diff_aug1'
):
    ...
    for item in tqdm(dataset.select(range(total))):
        ...
        with open(open_path, mode='r', newline='') as infile:
            ...
            for row in reader:
                ...
                if answer =='1':
                    log_likelihood = float(loglikelihood(temp_model, sentence,option1))-float(loglikelihood(temp_model, sentence,option2))
                elif answer =='2':
                    log_likelihood = float(loglikelihood(temp_model, sentence,option2))-float(loglikelihood(temp_model, sentence,option1))
                else:
                    print('11111111111')
