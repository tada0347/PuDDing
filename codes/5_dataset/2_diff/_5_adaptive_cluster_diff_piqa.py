...
def sleb(
    ...
    def loglikelihood(model,context: str, continuation: str):
        inputs = tokenizer(f"Question: {context}\nAnswer: {continuation}", return_tensors="pt").to('cuda')
        with torch.no_grad():
    ...
    for item in tqdm(dataset.select(range(total))):
        ...
        choice1 = item['sol1']
        choice2 = item['sol2']
        correct_choice = item['label'] + 1  # PIQA는 0-indexed
        ...
        with open(open_path, mode='r', newline='') as infile:
            ...
            for row in reader:
                ...
                if correct_choice ==1:
                    log_likelihood = float(loglikelihood(temp_model, context, choice1))-float(loglikelihood(temp_model, context, choice2))
                elif correct_choice ==2:
                    log_likelihood = float(loglikelihood(temp_model, context, choice2))-float(loglikelihood(temp_model, context, choice1))
                else:
        ...
        with open(result_path, mode='a',encoding="utf-8", newline='') as outfile:
            writer = csv.DictWriter(outfile, fieldnames=fieldnames)
            inputs1 = f"Question: {context}\nAnswer: {choice1}"
            new_row1 = {'inputs': inputs1, 'max_idx': max_idx, 'max_set': max_set,'softmax': softmax_values, 'minmax_softmax': softmax_minmax_values,'likelihood_diff': diff_list }
            inputs2 = f"Question: {context}\nAnswer: {choice2}"
            new_row2 = {'inputs': inputs2, 'max_idx': max_idx, 'max_set': max_set,'softmax': softmax_values, 'minmax_softmax': softmax_minmax_values,'likelihood_diff': diff_list }
