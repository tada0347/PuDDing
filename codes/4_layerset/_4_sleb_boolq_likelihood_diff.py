...
def get_loss(model, testenc,nsamples, bs=1, device=None):
    ...
    for item in tqdm(testenc.select(range(nsamples))):
        ...
        log_likelihood1= loglikelihood(model,passage, question, 'True')
        log_likelihood2= loglikelihood(model,passage, question, 'False')



        if label== True:
            float_sum+= float(log_likelihood1)-float(log_likelihood2)
        elif label == False:
            float_sum+= float(log_likelihood2)-float(log_likelihood1)
        else:
            print(sdfs)
