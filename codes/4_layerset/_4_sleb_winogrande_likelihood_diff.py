...
def get_loss(model, testenc,nsamples, bs=1, device=None):
    ...
    for item in tqdm(testenc.select(range(nsamples))):
        ...
        if answer =='1':
            float_sum+= float(log_likelihood_1)- float(log_likelihood_2)
            # print(float(torch.exp(log_likelihood_1))- float(torch.exp(log_likelihood_2)))
        elif answer=='2':
            float_sum+= float(log_likelihood_2)- float(log_likelihood_1)
            # print(float(torch.exp(log_likelihood_2))-float(torch.exp(log_likelihood_1)))
        else:
            print(szdfsdf)
    return -float_sum
