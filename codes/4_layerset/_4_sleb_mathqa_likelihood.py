...
#     log_probs = torch.log_softmax(logits, dim=-1)
#     log_likelihood = torch.gather(log_probs, dim=-1, index=input_ids.unsqueeze(-1))
#     log_likelihood = log_likelihood.squeeze(-1)  # shape: (seq_len,)
...
def get_loss(model, testenc,nsamples, bs=1, device=None):
    ...
    for item in tqdm(testenc.select(range(nsamples))):
        ...
        log_likelihood_1 = loglikelihood(model, problem, options, correct)
...
def sleb(
    ...
    # removal_list=[25, 26, 24, 23, 10, 22, 3]
    if eval_zeroshot:
        ...
        # tasks = ['arc_challenge','arc_easy','boolq']
        tasks = ['hellaswag']
        #########################
