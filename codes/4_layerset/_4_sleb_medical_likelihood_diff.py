...
def loglikelihood(model,question: str, context: str, answer: str):
    input_txt=f"{context}\nQuestion: {question}\nAnswer: {answer}"
...
def get_loss(model, testenc,nsamples, bs=1, device=None):
    ...
    for item in tqdm(testenc.select(range(nsamples))):
        ...
        log_likelihood_a = float(loglikelihood(model, question, context, 'yes'))
        log_likelihood_b = float(loglikelihood(model, question, context, 'no'))
        log_likelihood_c = float(loglikelihood(model, question, context, 'maybe'))

        ssumm=log_likelihood_a+log_likelihood_b+log_likelihood_c
        log_likelihoods = {
            'yes': log_likelihood_a,
            'no': log_likelihood_b,
            'maybe': log_likelihood_c,
        }

        answer = log_likelihoods[final_decision]
        log_likelihood = 3 * answer - ssumm
...
def sleb(
        ...
        seed: int = 0,
        nsamples: int = 128,
        result_folder: str = 'result/4_custom/pubmedqa',
    ...
    removal_list=  [30, 5, 23, 24, 29, 26, 17]
    if eval_zeroshot:
        ...
        # tasks = ['pubmedqa','mathqa','arc_challenge','arc_easy','boolq','piqa','winogrande']
        # tasks = ['arc_easy','boolq','piqa','winogrande']
        tasks = ['hellaswag']
        #########################
