...
def loglikelihood(model,context: str, continuation: str, correct: str):
    input_txt=f"Problem: {context}\nOptions: {continuation}\Answer: {correct}"
...
def get_loss(model, testenc,nsamples, bs=1, device=None):
    ...
    for item in tqdm(testenc.select(range(nsamples))):
        ...
        log_likelihood_a = float(loglikelihood(model, problem, options, 'a'))
        log_likelihood_b = float(loglikelihood(model, problem, options, 'b'))
        log_likelihood_c = float(loglikelihood(model, problem, options, 'c'))
        log_likelihood_d = float(loglikelihood(model, problem, options, 'd'))
        log_likelihood_e = float(loglikelihood(model, problem, options, 'e'))


        ssumm=log_likelihood_a+log_likelihood_b+log_likelihood_c+log_likelihood_d+log_likelihood_e
        log_likelihoods = {
            'a': log_likelihood_a,
            'b': log_likelihood_b,
            'c': log_likelihood_c,
        ...
        answer = log_likelihoods[correct]
        log_likelihood = 5 * answer - ssumm
...
def sleb(
        ...
        seed: int = 0,
        nsamples: int = 128,
        result_folder: str = 'result/4_custom/math_qa',
    ...
    removal_list=[18, 20, 19, 28, 27, 25, 21]
    ...
    if eval_zeroshot:
        ...
        # tasks = ['mathqa','arc_challenge','arc_easy','boolq','piqa','winogrande']
        tasks = ['hellaswag']
        #########################
