...
def build_multiple_choice_prompt(no1, no2, no3, answer):
    all_choices = [no1, no2, no3, answer]
    random.shuffle(all_choices) 

    correct_index = all_choices.index(answer)
    option_letters = ["A", "B", "C", "D"]
    options = [(option_letters[i], all_choices[i]) for i in range(4)]
    correct_option_letter = option_letters[correct_index]

    option_text = "Options:\n" + "\n".join([f"{letter}) {text}" for letter, text in options])
    
    return option_text, correct_option_letter
...
def loglikelihood(model,question: str, option_text: str, answer: str):
    input_txt=f"Question: {question}\n{option_text}\nAnswer: {answer}"
    inputs = tokenizer(input_txt, return_tensors="pt").to('cuda')
    ...
    for i in range(len(input_ids) - len(answer_tokens) + 1):
        ...
        raise ValueError('"Rationale:" not found in input text.')
...
def get_loss(model, testenc,nsamples, bs=1, device=None):
    ...
    for item in tqdm(testenc.select(range(nsamples))):
        ...
        option,answer=build_multiple_choice_prompt(distractor1,distractor2,distractor3,correct_answer)

        log_likelihood_a = float(loglikelihood(model, question, option, 'A'))
        log_likelihood_b = float(loglikelihood(model, question, option, 'B'))
        log_likelihood_c = float(loglikelihood(model, question, option, 'C'))
        log_likelihood_d = float(loglikelihood(model, question, option, 'D'))


        ssumm=log_likelihood_a+log_likelihood_b+log_likelihood_c+log_likelihood_d
        log_likelihoods = {
            'A': log_likelihood_a,
            'B': log_likelihood_b,
            'C': log_likelihood_c,
        ...
        answer_log = log_likelihoods[answer]
        log_likelihood = 4 * answer_log - ssumm
...
def sleb(
        ...
        seed: int = 77,
        nsamples: int = 128,
        result_folder: str = 'result/4_custom/sciq',
        result_file: str = 'likelihood_diff',
        dataset: str = 'sciq',
    ...
    # removal_list=[28, 23, 27, 21, 22, 19, 26]
    if eval_zeroshot:
        ...
        if '30b' or '66b' or '70b' in model_name:
            ...
            parallelize = False
        # tasks = ['sciq','pubmedqa','mathqa','arc_challenge','arc_easy','boolq','piqa','winogrande','hellaswag']
        tasks = ['sciq','arc_easy','arc_challenge','piqa']
