...
#     target_ids = inputs["input_ids"][:, 1:].contiguous()  # continuation 부분
#     logits = logits[:, :-1, :]  # 마지막 token에 대한 logits 제외
#     log_probs = torch.log_softmax(logits, dim=-1)
#     log_likelihood = torch.gather(log_probs, dim=-1, index=target_ids.unsqueeze(-1))
#     log_likelihood = log_likelihood.squeeze(-1)  # 차원 축소
...
def get_loss(model, testenc,nsamples, bs=1, device=None):
    ...
    for item in tqdm(testenc.select(range(nsamples))):
        ...
        log_likelihood_0 = loglikelihood(model,question, text,0)
        log_likelihood_1 = loglikelihood(model,question, text,1)
        log_likelihood_2 = loglikelihood(model,question, text,2)
        if len(text)==4:
            log_likelihood_3 = loglikelihood(model,question, text,3)
        ...
        if len(text)==4:
            if answer ==0:
                float_sum+= 3*softmax0-softmax1-softmax2-softmax3
            elif answer ==1:
                float_sum+= 3*softmax1-softmax0-softmax2-softmax3
            elif answer ==2:
                float_sum+= 3*softmax2-softmax1-softmax0-softmax3
            elif answer ==3:
                float_sum+= 3*softmax3-softmax1-softmax2-softmax0
            else:
                print(sdfsdf)

        else: 
            if answer ==0:
                float_sum+= 2*softmax0-softmax1-softmax2
            elif answer ==1:
                float_sum+= 2*softmax1-softmax0-softmax2
            elif answer ==2:
                float_sum+= 2*softmax2-softmax1-softmax0
            else:
                print(sdfsdf)
...
def sleb(
        ...
        seed: int = 0,
        nsamples: int = 128,
        result_folder: str = 'result/4_custom/arc_challenge',
        result_file: str = 'log_likelihood_diff',
        dataset: str = 'arc_challenge',
