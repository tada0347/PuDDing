...
def get_loss(model, testenc,nsamples, bs=1, device=None):
    ...
    for item in tqdm(testenc.select(range(nsamples))):
        ...
        # print()
        # print(sentence.replace("_", option1))
        # print(sentence.replace("_", option2))
        # log_likelihood_2 = loglikelihood(model, sentence.replace("_", option2))
        # print(f'1: {log_likelihood_1}, {float(torch.exp(log_likelihood_1))}')
        # print(f'2: {log_likelihood_2}, {float(torch.exp(log_likelihood_2))}')


        # predicted_choice = 1 if log_likelihood_1 > log_likelihood_2 else 2

        if answer =='1':
            log_likelihood_1 = loglikelihood(model, sentence,option1)
            float_sum+=  float(log_likelihood_1)
            # print(float(torch.exp(log_likelihood_1)))
        elif answer=='2':
            log_likelihood_2 = loglikelihood(model, sentence,option2)
            float_sum+=float(log_likelihood_2)
            # print(float(torch.exp(log_likelihood_2)))
        else:
            print(sdfsdfs)
    return -float_sum
...
def sleb(
        ...
        seed: int = 0,
        nsamples: int = 128,
        result_folder: str = 'result/4_custom/winogrande',
        result_file: str = 'likelihood_aug',
        dataset: str = 'winogrande',
