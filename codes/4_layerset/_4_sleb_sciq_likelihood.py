...
def get_loss(model, testenc,nsamples, bs=1, device=None):
    ...
    for item in tqdm(testenc.select(range(nsamples))):
        ...
        log_likelihood_1 = loglikelihood(model, question,option, answer)

        float_sum+= float(log_likelihood_1)
...
def sleb(
    ...
    # removal_list=[22, 28, 24, 26, 23, 19, 6]
    if eval_zeroshot:
        ...
        tasks = ['arc_easy','boolq','piqa','winogrande','hellaswag']
