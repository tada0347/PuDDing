...
@torch.no_grad()
def load_and_eval_ppl(model_name, model, device=torch.device("cuda:0"), dataset='wikitext2', testloader=None, tokenizer=None):
    # Print status
...
def eval_zero_shot(model_name, router_path=None,lora_path=None, task_list=['piqa','winogrande','hellaswag','arc_challenge','arc_easy'], 
    ...
    from lm_eval import tasks, v24_evaluator, utils
    ...
    results={}
    for task in task_list:
        result = v24_evaluator.simple_evaluate(
            model='hf',
