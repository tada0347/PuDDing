...
@torch.no_grad()
def load_and_eval_ppl(model_name, model, device=torch.device("cuda:0"), dataset='wikitext2', testloader=None, tokenizer=None,portion=None,use_last_fraction=0.5):
    # Print status
...
def eval_ppl(model, testenc, bs=1, device=None):
    ...
    nlls = []
    print(f'testenc.numel() {testenc.numel()}')
    ...
    print(f'total tokens: {nsamples * model.seqlen}')
...
def eval_ppl_batch_input_prompt_portion(model, data, bs=1, device=None,use_last_fraction=0.5):
    ...
    for i in tqdm(range(0, nsamples, bs)):
        ...
        lm_logits = model(inputs).logits  # (batch_size, seq_len, vocab_size)
        shift_logits = lm_logits[:, :-1, :].contiguous()
        shift_labels = inputs[:, 1:]

        loss = loss_fct(shift_logits.view(-1, shift_logits.size(-1)), shift_labels.view(-1))
        loss = loss.view(j - i, model.seqlen - 1)  # (batch_size, seq_len-1)
        ...
        while sentence_start_idx < len(sentence_lengths) and token_idx < (j * model.seqlen):
            ...
            # trunc_start = start_idx + int(0.2 * (end_idx - start_idx))
...
# def eval_ppl_per_sentence(model, testenc_list, bs=1, device=None):
    
#     nlls = []
#     total_tokens=0

#     for testenc in tqdm(testenc_list):
#         inputs = testenc.input_ids.to(device)
...
torch.no_grad()
def eval_zero_shot(model_name, router_path=None,lora_path=None, task_list=['piqa','winogrande','hellaswag','arc_challenge','arc_easy'], 
        num_fewshot=0, parallelize=False):
    
    from lm_eval import tasks, v23_evaluator, utils
    ...
    results={}
    for task in task_list:
        result = v23_evaluator.simple_evaluate(
            model='hf',
